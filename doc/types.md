...
The Other .NET types column specifies which other data types in which the value can be retrieved using NpgsqlDataReader.GetBoolean, .GetByte, .GetDouble etc. or by .GetFieldValue<T>.

## Type mappings when sending parameters to the backend

There are three rules that decides what backend type a parameter will sent as. First, the NpgsqlDbType and DbType of the parameter is inspected. If no such is set, the backend type will be inferred based on the type the value has. For DateTime and NpgsqlDateTime, the Kind attribute tells wether to use timestamp or timestamptz.
When NpgsqlDbType or DbType is set to a primitive type (bool, numbers and string), most other primitive types are accepted since they all implement the IConvertible interface, which is what Npgsql uses to convert the value to the target type.
