...
* Multiple improvements have been made to SSL/TLS support, and the [`Ssl Mode`](https://www.npgsql.org/doc/connection-string-parameters.html#security-and-encryption) connection string parameter has been aligned with the standard PostgreSQL [`ssl_mode`](https://www.postgresql.org/docs/current/libpq-ssl.html#LIBPQ-SSL-SSLMODE-STATEMENTS) setting ([#4006](https://github.com/npgsql/npgsql/issues/4006)). See breaking change note below for `SSL Mode=Require`.
...
* To validate server certificates, use `SSL Mode=VerifyFull` (or alternatively SSL `VerifyCA`) instead of `Require`.
  * `VerifyFull`/`VerifyCA` will perform certificate revocation checking (`Require` did not do this by default). If you need to turn this off for smoe reason, specify `Check Certificate Revocation=false`.
