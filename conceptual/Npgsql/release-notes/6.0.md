...
Npgsql can now report tracing data via OpenTelemetry; this can provide invaluable data on the queries your application is running, how long they're taking, and situate them in the larger context of your application. For example, you can visualize follow what percentage of your HTTP requests are spent waiting for your database query.
...
PostgreSQL has special `infinity` and `-infinity` values for timestamps and dates, which are bigger and smaller than other value. Npgsql has supported mapping `DateTime.MaxValue` and `MinValue` to these infinity values via an `Convert Infinity DateTime` connection string parameter, which was disabled by default. This behavior is now on by default, since `DateTime.MaxValue` and `MinValue` are very rarely used as actual timestamps/dates; set `Convert Infinity DateTime` to revert to the previous behavior.
