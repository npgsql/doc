...
* Introduce a non-caching password provider callback via <xref:NpgsqlDataSourceBuilder.UsePasswordProvider?displayProperty=nameWithType>.
* Allow customizing System.Text.Json JsonSerializationOptions via <xref:NpgsqlDataSourceBuilder.ConfigureJsonOptions?displayProperty=nameWithType>.
...
* Allow providing the root certificate programmatically via the new <xref:NpgsqlDataSourceBuilder.UseRootCertificate?displayProperty=nameWithType>
